...
@measure_time
def process_drama_id(drama_id, previous_paid_uids):
    ...
    if sound_lists:
        for sound in sound_lists:
            ...
            print(f"Loading {sound_detail['sound_title']}, {sound_detail['create_time']}")
    ...
    print("----------------------------------------Summary------------------------------------------")
    print("声音标题, 创建时间, 是否需要付费, 弹幕用户ID, 评论用户ID, 总用户ID, 观看次数, 每集新增付费ID")
    for sound_detail in sound_data:
        ...
        # Print sound detail
        print(f"{sound_detail['sound_title']}, {sound_detail['create_time']}, {'PAID' if int(sound_detail['need_pay']) > 0 else 'FREE'}, "
              f"{len(sound_detail['danmaku_uids'])}, {len(sound_detail['comment_uids'])}, {len(sound_detail['total_sound_uids'])}, "
              f"{sound_detail['view_count']}, {len(new_paid_uids)}")

    # Print two new rows after the sound data
    print(f"End of data for drama ID {drama_id}")
    print('')

    print("剧集ID, 剧集名称, 首个声音创建时间, 价格, 总观看次数, 付费观看次数, 免费观看次数, 付费弹幕用户ID, 付费评论用户ID, 免费弹幕用户ID, 免费评论用户ID, 付费总用户ID, 免费总用户ID, 前五十打赏, 新增付费用户增长")
    print(f"{drama_id}, {name}, {first_sound_create_time}, {price}, {view_count}, {paid_view_count}, {free_view_count}, "
          f"{len(total_paid_danmaku_udis)}, {len(total_paid_comment_uids)}, {len(total_free_danmaku_udis)}, "
          f"{len(total_free_comment_uids)}, {len(total_paid_udis)}, {len(total_free_udis)}, {fetch_top_50_coin}, {paid_uids_growth}")
...
def runner():
    ...
    for drama_id in DramaIds:
        sound_data, total_paid_udis = process_drama_id(drama_id.strip(), previous_paid_uids)
